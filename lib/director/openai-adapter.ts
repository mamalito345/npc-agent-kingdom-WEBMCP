...
function extractOutputText(
  ...
  if (
    ...
    throw new Error(
      "OPENAI_RESPONSE_TEXT_NOT_FOUND"
    );
  }
...
function configuredDirectorModel(
  explicit:
    string |
    undefined,
  injectedTransport:
  ...
  /*
   * Historical deterministic smoke tests inject a fake transport and do not
   * send the request to OpenAI. Keep those tests independent from deployment
   * environment variables without inventing a real OpenAI model identifier.
   */
  if (
    injectedTransport
  ) {
    return "injected-transport-model";
  }
...
export class OpenAIEventDirectorAdapter
  implements
    EventDirectorModelAdapter {
  ...
  async selectEvent(
        ...
        .request({
          ...
          text: {
            format: {
              ...
              name:
                "world_director_event_selection",
    ...
    if (
      ...
      throw new Error(
        "INVALID_EVENT_DIRECTOR_RESPONSE"
      );
    }
...
export class OpenAIWorldDirectorProposalAdapter
  implements
    DirectorModelAdapter {
  private readonly transport:
    OpenAIResponsesTransport;
  ...
  async generateProposals(
    ...
    if (
      ...
      throw new Error(
        "INVALID_DIRECTOR_PROPOSAL_RESPONSE"
      );
    }
